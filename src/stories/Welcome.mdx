...
import generateColorShades from "../shared/utils/generateColorShades";

export const color_scheme = [
    { name: 'Primary', color: '#E27396', },
    { name: 'Secondary', color: '#B1DEE2', },
    { name: 'Tertiary', color: '#EBF3D8', }
].map(({ color, ...rest }) => ({ color, shades: generateColorShades(color), ...rest }));

<table>
    {color_scheme.map(({name, color, shades}) => (
        <tr>
            <td className="palette-cell" style={{ backgroundColor: shades[0] }}>{ name }</td>
            {shades.map((shade) => (
